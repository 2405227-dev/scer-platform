...
interface ModuleNavbarProps {
  currentModule?: "scer" | "geo" | "live-response";
}
...
const modules = [
  {
    id: "scer",
    label: "Command",
    href: process.env.NEXT_PUBLIC_SCER_API_URL ? `${process.env.NEXT_PUBLIC_SCER_API_URL}/command` : "http://localhost:3000/command",
...
export function ModuleNavbar({ currentModule = "live-response" }: ModuleNavbarProps) {
  const [open, setOpen] = useState(false);

  const scerUrl = process.env.NEXT_PUBLIC_SCER_API_URL || "http://localhost:3000";
      ...
      <header className="fixed left-0 right-0 top-0 z-[9999] px-3 pt-3 sm:px-5">
        <div className="mx-auto max-w-[1550px]">
          <div className="relative flex h-[62px] items-center rounded-[22px] border border-white/[0.08] bg-[#07101b]/90 px-3 shadow-[0_20px_80px_rgba(0,0,0,.4)] backdrop-blur-2xl sm:px-4">
            ...
            >
              <div className="relative grid h-9 w-9 place-items-center rounded-xl border border-cyan-400/20 bg-cyan-400/[0.08]">
                <Zap className="h-[18px] w-[18px] text-cyan-300 transition group-hover:scale-110" />
                <span className="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full bg-cyan-400 animate-pulse ring-2 ring-[#07101b]" />
              </div>
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="rounded-md bg-cyan-500/20 px-1.5 py-0.5 text-[8px] font-black uppercase text-cyan-300 border border-cyan-500/30">
                    Port 3004
                  </span>
                </div>
                <div className="text-[7px] font-bold uppercase tracking-[0.14em] text-slate-500">
                  SCER Dedicated Responder Center
                </div>
              </div>
            </a>
            ...
            <div className="hidden md:flex items-center ml-4">
              ...
              >
                ...
                <span>Command Center (Port 3000)</span>
              </a>
            </div>
            ...
            <div className="ml-auto hidden items-center gap-2 lg:flex">
              ...
              <div className="flex items-center gap-2 rounded-xl border border-cyan-400/10 bg-cyan-400/[0.035] px-3 py-2">
