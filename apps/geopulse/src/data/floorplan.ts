/**
 * Campus 25 & Academic Blocks Floor Plan Dataset (from ATLAS Incident Map)
 *
 * Real transcribed layout: three wings (A, B, C) meeting at a central circulation
 * spine, room runs, academic offices, lecture theatres, lifts, stairs and amenities.
 * Coordinates are in 0-100 plan space.
 */
...
export interface FloorSpace {
  ...
  note?: string;
  /**
   * Floors this space exists on. Omitted means every floor.
   */
...
export const FLOOR_SPACES: readonly FloorSpace[] = [
  ...
  room('B 206', 24, 14, 9, 6, 'B', 'room', 'Computer Systems Lab 2'),
  ...
  room('C 203', 76, 46, 20, 8, 'C', 'office', 'Society Office · Cyber Intelligence, QA Cell'),
  room('C 211', 76, 62, 11, 6, 'C', 'lecture', 'Lecture Theatre 4'),
  room('C 213', 63, 62, 11, 6, 'C', 'lecture', 'Lecture Theatre 6'),
  room('C 212 W', 63, 74, 11, 7, 'C', 'lecture', 'Lecture Theatre 5-A'),
  room('C 212 E', 78, 74, 11, 7, 'C', 'lecture', 'Lecture Theatre 5-B'),
  { ...room('Library', 84, 38, 12, 6, 'C', 'amenity', 'Towards Central Library'), onlyFloors: [3] },
  ...
  room('Seating', 70, 82, 8, 5, 'C', 'amenity', 'Student Commons & Lounge'),
...
export const FLOOR_WINGS = [
  { id: 'A' as const, label: 'A Block (South Wing)', hint: 'Rooms A 101–218 · Labs & Seminar Halls' },
  { id: 'B' as const, label: 'B Block (North Wing)', hint: 'Rooms B 101–222 · Engineering Dept' },
  { id: 'C' as const, label: 'C Block (East Wing)', hint: 'Faculty Chambers, Theatres & Society' },
];

export const CAMPUS_FLOORS = [
  { id: 1, label: 'Floor 1 (Ground Level)', surveyed: false },
  { id: 2, label: 'Floor 2 (Main Academic)', surveyed: true },
  { id: 3, label: 'Floor 3 (Research & Labs)', surveyed: false },
] as const;
...
export function spacesForFloor(floor: FloorId): FloorSpace[] {
  ...
  if (floor === 2) return present as FloorSpace[];
